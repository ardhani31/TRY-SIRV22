...
class Scenario(Term):
    ...
    def _convert_variables(self, abbr, candidates):
        ...
        """
        if abbr is None:
            return [self.CI, self.F, self.R, self.V]
        if abbr == "all":
            return self._ensure_list(candidates, name="candidates")
        abbr_dict = {"C": self.C, "I": self.CI, "F": self.F, "R": self.R, "V": self.V,}
    ...
    def estimate_accuracy(self, phase, name="Main", **kwargs):
        ...
        Note:
            If 'Main' was used as @name, main PhaseSeries will be used.
        """
        variables = [self.CI, self.F, self.R, self.V]
    ...
    def _describe(self):
        ...
        for (name, tracker) in self._tracker_dict.items():
            ...
            # Vaccinated on the next date of the last phase
            last_v = df.loc[last_date, self.V]
            ...
            _dict[name] = {
                ...
                f"{self.V} on {last_date_str}": last_v,
            }
    ...
    def score(self, variables=None, name="Main", **kwargs):
        ...
        variables = self._ensure_list(
            variables or [self.CI, self.F, self.R, self.V],
            candidates=[self.S, self.C, self.CI, self.F, self.R, self.V], name="variables")
        # Specify date range
        start, end = tracker.parse_range(**find_args(PhaseTracker.parse_range, **kwargs))
    ...
    def restore(self, filename):
        ...
        model_dict = {SIR.NAME: SIR, SIRD.NAME: SIRD, SIRF.NAME: SIRF, SIRV.NAME: SIRV}
